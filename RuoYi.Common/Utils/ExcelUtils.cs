...
#region Import by stream
        ...
        public static async Task<List<T>> ImportAllAsync<T>(MemoryStream stream, string? startCell = "A1")
            where T : class, new()
        {
            var list = new List<T>();
            var config = GetOpenXmlConfiguration<T>(ExcelOperationType.Import);
...
#region Import by filePath
        public static async Task<IEnumerable<T>> ImportAsync<T>(string filePath, string? sheetName = null)
            where T : class, new()
        {
            return await MiniExcel.QueryAsync<T>(filePath, sheetName, ExcelType.XLSX);
        }

        public static async Task<List<T>> ImportAllAsync<T>(string filePath)
            where T : class, new()
        {
            var list = new List<T>();
...
#endregion
#region Export
        public static async Task GetImportTemplateAsync<T>(HttpResponse response, string? sheetName = null)
            where T : class, new()
        {
        ...
        public static async Task ExportAsync<T>(HttpResponse response, IEnumerable<T> list, string? fileName = null, string? sheetName = null, ExcelOperationType optType = ExcelOperationType.Export)
            where T : class, new()
        {
