    ...
    public class SysUserRepository : BaseRepository<SysUser, SysUserDto>
    {
        ...
        public override ISugarQueryable<SysUser> Queryable(SysUserDto dto)
        {
            ...
            return Repo.AsQueryable()
                ...
                //.WhereIF(!string.IsNullOrEmpty(dto.UserName),
                // u => SqlFunc.MappingColumn(u.UserName,$"{nameof(u.UserName)} COLLATE utf8mb4_general_ci").Contains(dto.UserName!))//解决字符不兼容
               ...
               //           u => SqlFunc.MappingColumn(u.UserName,$"{nameof(u.UserName)} COLLATE utf8mb4_general_ci").Contains(dto.UserName!))//解决字符不兼容
                ...
                .WhereIF(dto.DeptId > 0,(u) => u.DeptId == dto.DeptId
                        || u.DeptId == SqlFunc.Subqueryable<SysDept>()
                            .Where(d => SqlFunc.SplitIn(
                                SqlFunc.MappingColumn(d.Ancestors,$"{nameof(d.Ancestors)} COLLATE utf8mb4_general_ci"),
                                dto.DeptId.ToString()
                            ))
                            .GroupBy(d => d.DeptId)
                            .Select(d => d.DeptId)
