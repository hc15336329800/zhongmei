...
namespace RuoYi.System.Repositories
{
    public class SysUserPostRepository : BaseRepository<SysUserPost, SysUserPostDto>
    {
        ...
        {
            return Repo.AsQueryable().WhereIF(dto.UserId > 0, (t) => t.UserId == dto.UserId).WhereIF(dto.PostId > 0, (t) => t.PostId == dto.PostId);
        }

        public override ISugarQueryable<SysUserPostDto> DtoQueryable(SysUserPostDto dto)
        {
            return Repo.AsQueryable().WhereIF(dto.UserId > 0, (t) => t.UserId == dto.UserId).WhereIF(dto.PostId > 0, (t) => t.PostId == dto.PostId).Select((t) => new SysUserPostDto { UserId = dto.UserId, PostId = dto.PostId });
        }
