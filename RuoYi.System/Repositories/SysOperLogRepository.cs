...
public class SysOperLogRepository : BaseRepository<SysOperLog, SysOperLogDto>
{
    ...
    {
        return Repo.AsQueryable().WhereIF(dto.OperId > 0, (t) => t.OperId == dto.OperId).WhereIF(dto.Params.BeginTime != null, (u) => u.OperTime >= dto.Params.BeginTime).WhereIF(dto.Params.EndTime != null, (u) => u.OperTime <= dto.Params.EndTime).WhereIF(dto.OperId > 0, (t) => t.OperId == dto.OperId);
    }

    public override ISugarQueryable<SysOperLogDto> DtoQueryable(SysOperLogDto dto)
    {
        return Repo.AsQueryable().WhereIF(dto.OperId > 0, (t) => t.OperId == dto.OperId).WhereIF(dto.Params.BeginTime != null, (u) => u.OperTime >= dto.Params.BeginTime).WhereIF(dto.Params.EndTime != null, (u) => u.OperTime <= dto.Params.EndTime).Select((t) => new SysOperLogDto { OperId = t.OperId }, true);
    }
